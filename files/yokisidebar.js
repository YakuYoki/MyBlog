function milusidebar(setting) {
  var c={
    ...
    "icon":"#icon-QQ","url":"http://wpa.qq.com/msgrd?v=3&uin=978761587&site=qq&menu=yes",
    "title":"QQ"
    },{
      "icon":"#icon-juejin",
  ...
  function sidebar(c){
    ...
    var p_b_follow=$("#p_b_follow").html();$("#profile_block").before('<div class="attention" onclick="follow(\''+c.follow+"')\"><span>+加关注</span></div>");
    ...
    for(var i=0;i<le;i++){
      sidebarInfoHtml+="<tr>";for(var j=0;j<sidebarInfo[i].length;j++){
        if(sidebarInfo[i][j].click==false){
          sidebarInfoHtml+='<td><svg class="icon '+sidebarInfo[i][j].classname+'" aria-hidden="true"><use xlink:href="'+sidebarInfo[i][j].icon+'"></use></svg></td>'}else{
        ...
        sidebarInfoHtml+="</tr>"}sidebarInfoHtml+='</table><p class="catListTitle" style="margin-bottom:20px">'+c.signature+"</p>";
        $("#blog-news").append(sidebarInfoHtml);
        $("#blog-calendar").before('<h3 class="catListTitle">日历</h3>');
        tippy(".popper_weixin",{
          content:c.popper_weixin,theme:"tomato",
          allowHTML:true,
          animation:"scale",
          duration:500,
          arrow:true,
